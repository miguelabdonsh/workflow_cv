...
interface MultiCVChatPanelProps {
  results: AnalysisResult[];
  onClose: () => void;
  jobDescription: string;
}

const MultiCVChatPanel: React.FC<MultiCVChatPanelProps> = ({ results, onClose, jobDescription }) => {
  ...
  useEffect(() => {
    const initializeChat = async () => {
      try {
        console.log("Iniciando sesión de chat global para todos los CVs");
        const response = await fetch('http://localhost:8000/chat/multi-sessions/', {
          method: 'POST',
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Error al crear la sesión de chat multi-CV');
        }
        ...
        const welcomeMessage: ChatMessage = {
          role: 'assistant',
          content: `Hola, soy tu asistente para analizar los ${results.length} perfiles de candidatos. Puedo responder preguntas comparativas o sobre candidatos específicos. ¿En qué puedo ayudarte?`,
          timestamp: new Date()
        };
    ...
    return () => {
      if (sessionId) {
        console.log("Cerrando sesión de chat global:", sessionId);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl h-[600px] max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="border-b border-gray-200 px-4 py-3 flex justify-between items-center">
          <h3 className="text-lg font-semibold text-gray-800">
            Chat sobre los {results.length} Candidatos
          </h3>
          <button 
            onClick={onClose}
...
export default MultiCVChatPanel; 